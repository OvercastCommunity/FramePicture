...
public class FrameListener implements Listener {
  ...
  public void onPlayerInteract(PlayerInteractEntityEvent event) {
    ...
    if (Cache.hasCacheCreating(player)) {
      ...
      PictureDatabase.FinishDownloadSignal signal =
          new PictureDatabase.FinishDownloadSignal() {
            ...
            public void downloadSuccess(File file, boolean wasLocal) {
              ...
              if (frame == null) {
                player.sendMessage(
                    Lang.PREFIX.getText() + Lang.NO_PICTURE.getText().replace("%url", path));
            ...
            public void downloadError(Exception e) {
              player.sendMessage(
                  Lang.PREFIX.getText() + Lang.DOWNLOAD_ERROR.getText().replace("%url", path));
            }
          };
      ...
      if (new File(this.manager.getPictureDatabase().getOutputFolder(), path).exists()) {
        signal.downloadSuccess(
            new File(this.manager.getPictureDatabase().getOutputFolder(), path), true);
      } else {
    ...
    // MULTI CREATING
      ...
      ItemFrame cacheFrame =
          Utils.getItemFrameFromChunk(
              entity.getLocation().getChunk(), new Location(world, x, y, z), entity.getFacing());
      ...
      PictureDatabase.FinishDownloadSignal signal =
          new PictureDatabase.FinishDownloadSignal() {
            ...
            public void downloadSuccess(File file, boolean wasLocal) {
              BufferedImage image =
                  FrameListener.this.manager.getPictureDatabase().loadImage(file.getName());
              if (image == null) {
                player.sendMessage(
                    Lang.PREFIX.getText() + Lang.NO_PICTURE.getText().replace("%url", path));
          ...
          Lang.PREFIX.getText()
              + Lang.GET_URL
                  ...
                  .replace(
                      "%entity",
                      (frame.isLoaded()
                          ? String.valueOf(frame.getEntity().getEntityId())
                          : "Not loaded"))
                  .replace("%id", String.valueOf(frame.getId())));
  ...
  public void onHangingBreak(HangingBreakEvent event) {
    ...
    if (player != null)
      player.sendMessage(
          Lang.PREFIX.getText()
              + Lang.FRAME_REMOVED.getText().replace("%id", String.valueOf(frame.getId())));
